...
Check mention velocity, influencer clustering, sentiment tone. If X tools fail, log the error and continue — but you MUST attempt the call.
...
2. If metadata contains a `website` field, fetch it: `web_fetch_url({ url: "<website_url>" })`
...
| Take-profit levels | [50%, 100%, 200%] | [25%, 50%, 100%] |
...
|---|---|---|
| < +25% | Original `trailingStopPct` | Standard buffer |
...
Trailing stops only tighten — never widen (except house money transition). Match current unrealized PnL, not peak.

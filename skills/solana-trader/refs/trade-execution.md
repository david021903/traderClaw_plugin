...
- `slippageBps` (scaled to liquidity, hard cap 800bps)
- `slPct`, `tpLevels`, `trailingStopPct` (simple) or `trailingStop` object (structured — see refs/api-reference.md § Trailing Stop Parameter)
- `managementMode`
...
As unrealized profit grows, tighten trailing stops to lock in gains. Apply the highest matching level:

| Profit Level | Trailing Stop | Rationale |
|---|---|---|
| < +25% | Use original `trailingStopPct` from exit plan | Standard volatility buffer |
...
**Structured trailing stop (preferred):** Use the `trailingStop` object parameter instead of `trailingStopPct` for new trades:
```json
{
  "trailingStop": {
    "percentage": 15,
    "triggerAboveATH": false
  }
}
```
As profit levels change, update the `percentage` field using the level table above. See refs/api-reference.md § Trailing Stop Parameter for full schema.

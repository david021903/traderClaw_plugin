...
Install the **npm package** **`traderclaw-team-v1`**. The **OpenClaw plugin id** in `openclaw.json` stays **`solana-trader`** (same as `openclaw.plugin.json`). The global CLI binary is **`traderclaw`**.

```bash
npm install -g traderclaw-team-v1
```

Or install directly into OpenClaw:

```bash
openclaw plugins install traderclaw-team-v1
```

**Names:** This team edition publishes as **`traderclaw-team-v1`** on npm. The public **`solana-traderclaw`** package remains the default TraderClaw build; use this package when you need X posting + journal tools. OpenClaw may log a benign id hint if the npm package name and manifest `id` differ — config keys remain **`plugins.entries.solana-trader`**.
